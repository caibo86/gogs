...
{{range .Methods}}    {{symbol .Name}}({{params "arg" .Params false}})({{params "ret" .Return true}}){{"\n"}}{{end}}}
...
func (service *{{$Service}}Service) Call(call *network.Call) (callReturn *network.Return, err error) {
    ...
    switch call.MethodID { {{range .Methods}}
		...
		{{end}} {{paramsName "arg" .Params true}} = ReadParams{{$Service}}{{$Name}}(call.Params)
		if err != nil {
			return
		} {{end}}
		log.Debugw("call "+{{$Service}}MethodName(call.MethodID) {{paramsW "arg" .Params}})
		{{range .Return}} var ret{{.ID}} {{typeName .Type}}
		{{end}} {{paramsName "ret" .Return true}} = service.I{{$Service}}.{{$Name}}({{paramsName "arg" .Params false}})
        if err != nil {
            return
        }
		...
		callReturn.Params = WriteReturn{{$Service}}{{$Name}}({{paramsName "ret" .Return false}})
       	{{end}}return{{end}}
	}
...
func (service *{{$Service}}Service){{$Name}}({{params "arg" .Params false}})({{params "ret" .Return true}}){
    call := &network.Call{
        ServiceID: uint32(service.id),
        MethodID: {{.ID}},
    }
	{{if .Params}}call.Params = WriteParams{{$Service}}{{$Name}}({{paramsName "arg" .Params false}}){{end}}	
    ...
    select {
        case callReturn := <- future:
			{{paramsName "ret" .Return true}} = ReadReturn{{$Service}}{{$Name}}(callReturn.Params)
			if err != nil {
				return	
			}
...
func (service *{{$Service}}RemoteService){{$Name}}({{params "arg" .Params false}})({{params "ret" .Return true}}){
	call := &network.Call{
...
{{if .Params}}
// WriteParams{{$Service}}{{$Name}} is an autogenerated function, write the params of the method to a byte slice
func WriteParams{{$Service}}{{$Name}}({{params "arg" .Params false}})[]byte {
    var data []byte
	var n int
	var l int 
	_ = l
...
// ReadParams{{$Service}}{{$Name}} is an autogenerated function, read the params of the method from a byte slice
func ReadParams{{$Service}}{{$Name}}(data []byte)({{params "arg" .Params true}}) {
	defer func(){
		if e := recover(); e != nil {
			err = e.(error)
		}
...
{{if .Return}}
// WriteReturn{{$Service}}{{$Name}} is an autogenerated function, write the return of the method to a byte slice
func WriteReturn{{$Service}}{{$Name}}({{params "ret" .Return false}})[]byte {
    var data []byte
	var n int
	var l int 
	_ = l
...
// ReadReturn{{$Service}}{{$Name}} is an autogenerated function, read the return of the method from a byte slice
func ReadReturn{{$Service}}{{$Name}}(data []byte)({{params "ret" .Return true}}) {
	defer func(){
		if e := recover(); e != nil {
			err = e.(error)
		}
...
// {{$Service}}MethodName is an autogenerated function, returning the method name of service by the method id 
func {{$Service}}MethodName(methodID uint32) string { {{$SPath := .Path}}
	switch methodID { {{range .Methods}}
	case {{.ID}}:
		return "{{$SPath}}.{{.Name}}"{{end}}
	}
	return ""
}
