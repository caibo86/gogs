...
func (system *ActorSystem) NewActor(name ActorName, context IActorContext) (IActor, error) {
	...
	if !ok {
		return nil, gserrors.Newf("actor builder not found: %s", name.Type)
	}
	if context == nil {
	...
	if name.SystemName == "" || name.SystemName == system.name {
		...
		if old, ok := system.actors[nameStr]; ok {
			...
			return old, gserrors.Newf("actor: %s already exist", actor.Name())
		}
	...
	return nil, gserrors.Newf("neighbor actor system not found: %s", name.SystemName)
}

// ActorInvoke 角色调用
func (system *ActorSystem) ActorInvoke(msg *ActorMsg) (*gsnet.Return, Err, error) {
	system.actorLock.RLock()
	...
	call, err := gsnet.UnmarshalCall(msg.Data)
	if err != nil {
