...
type ServiceListener func(service IService, status gsnet.ServiceStatus) bool
...
type eventServiceStatusChanged struct {
	...
	status  gsnet.ServiceStatus // 需要监听的状态
}
	...
	for event := range publisher.events {
		...
		case *eventListenService:
			...
			for name, service := range publisher.services {
				if e.name == name {
					if !e.listener(service, gsnet.ServiceStatusOnline) {
						mark = false
					}
				}
			}
		...
		case *eventServiceStatusChanged:
			...
			if e.status == gsnet.ServiceStatusOnline {
...
func (publisher *ServiceStatusPublisher) ServiceStatusChanged(service IService, status gsnet.ServiceStatus) {
