...
// 游戏服
// 哈哈
@ServiceAttr(ID:100, Name:"game")
service GameServer(gss.MapServer) {
	GetServerTime(int32)       -> (int64, ErrCode);            // 无聊 获取服务器时间 单位是毫秒
	GetCarInfo(int32, Student) -> (Car, gss.Teacher, ErrCode); // 这个注释去哪里了 获取汽车信息
	HeartBeat();                                               // 心跳第一个注释 心跳第二个注释
}
